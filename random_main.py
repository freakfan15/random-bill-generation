...
detailed_bills_random = distribute_stock_items_randomly(allocated_sales, stocks, ledgers_with_allocated_sales)


# Display the detailed bills
for bill in detailed_bills_random[:5]:
    print(bill)
...
with open('detailed_billsx.csv', 'w', newline='') as csvfile:
    fieldnames = ['ledger_name', 'date', 'item_name', 'quantity', 'rate', 'total_sales']
    writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
    writer.writeheader()
    for bill in detailed_bills_random:
        for item in bill['items']:
            writer.writerow({**item, 'ledger_name': bill['ledger_name'], 'date': bill['date']})